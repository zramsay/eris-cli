...
yum_repo_path=/etc/yum.repos.d/eris_eris.repo
yum_repo_config_url="https://packagecloud.io/install/repositories/eris/eris/config_file.repo?os=${os}&dist=${major_version}&name=${host}&source=script"
...
yum install -y pygpgme --disablerepo='eris_eris'
...
if [ "$?" != "0" ]; then
  ...
  sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/eris_eris.repo
fi

echo "Installing yum-utils..."
yum install -y yum-utils --disablerepo='eris_eris'
...
echo "Generating yum cache for eris_eris..."
yum -q makecache -y --disablerepo='*' --enablerepo='eris_eris'