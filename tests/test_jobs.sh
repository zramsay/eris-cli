...
run_test(){
  ...
  then
    ...
    cat readme.md
    echo
    $cli_exec run --chain "$chain_name" --address "$key1_addr" --set "addr1=$key1_addr" --set "addr2=$key2_addr" --set "addr2_pub=$key2_pub" --debug
  else
    echo
    cat readme.md
    echo
    $cli_exec run --chain "$chain_name" --address "$key1_addr" --set "addr1=$key1_addr" --set "addr2=$key2_addr" --set "addr2_pub=$key2_pub"
  fi
  test_exit=$?
